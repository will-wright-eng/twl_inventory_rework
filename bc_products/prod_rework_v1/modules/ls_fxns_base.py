...
def extract_and_dict(file_list, master_list_csv, filename1, filename0):
    ...
    temp1 = [
        list(df1.custom_sku).index(i) for i in prod_ids
        if i in list(df1.custom_sku)
    ]
    temp2 = [i for i in prod_ids if i in list(df1.custom_sku)]
    categories = [
        list(df0.loc[df0.product_code_sku == i].category_0)[0] for i in temp2
    ]
...
def gen_prod_matrix(df, id_dict, mfg_code, prod, color_dicts, weight_dict):
    ...
    if category == 'Disc Golf':
        ...
        attributes = [[dg_color_dict[i][0], weight_dict[j][0]]
                      for i in range(len(dg_color_dict))
        ...
        sku_codes = [
            ...
            category == 'Sky-Styler Color Options') or (
    ...
    if return_flag:
        if len(mcols) == 1:
            matrix['description'] = matrix[
                'matrix_description'] + ' (' + matrix['attribute_1'] + ')'
        elif len(mcols) == 2:
            matrix[
                'description'] = matrix['matrix_description'] + ' (' + matrix[
                    'attribute_1'] + ' ' + matrix['attribute_2'] + ')'
