...
def pull_list_of_prods(filename, index_by, category_col):
    ...
    prod_ids = pd.Series(df[category_col].values,
                         index=df[index_by].values).to_dict()
    return prod_ids
...
def gen_prod_id_dict(filename, index_by, category_col, test):
    if test:
        ...
        test_sku_categories = [
            'Disc Golf', 'Ultimate Frisbee', 'Freestyle Frisbee'
        ]
...
def gen_import_table_with_skus_v2(df, prod_ids, id_dict, index_by, stock_field,
                                  mfg_code, file_list):
    ...
    for prod in list(prod_ids):
        ...
        rows_sku, return_flag = sku_struct_v2(master_sku, mfg_code, sku_ids,
                                              stock_total, category,
                                              color_dicts, weight_dict,
                                              stock_field, index_by)
        if return_flag:
