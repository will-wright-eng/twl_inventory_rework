...
def main():
	...
	df, id_list, id_dict, df, cols_dict = full_inventory_df_info(filename2,index_by,prod_ids)
	# loop through discraft product list and pull sequence of skus under each product in list
	mfg_code = 'DIS'
	ndf = gen_import_table_with_skus_v2(df,prod_ids,id_dict,index_by,stock_field,mfg_code,file_list)
